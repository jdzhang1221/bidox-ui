...
async function handleBatchReview(status: number) {
  ...
  if (rows.length === 0) {
    ElMessage.warning('所选凭证不符合当前审核操作');
    return;
  }
  try {
    await confirm(
...
async function handleBatchDelete() {
  ...
  if (rows.some((row) => row.status === FMS_VOUCHER_STATUS.APPROVED)) {
    ElMessage.warning('批量删除不能包含已审核凭证');
    return;
  }
  try {
  ...
  <Page auto-content-height>
    ...
    <Grid class="fms-voucher-list-grid">
          ...
          <template #dropdown>
            <ElDropdownMenu>
              <ElDropdownItem
                ...
                @click="handleMoreCommand('print')"
              >
                打印凭证
              </ElDropdownItem>
              <ElDropdownItem
                v-if="hasAccessByCodes(['fms:voucher:print'])"
                @click="handleMoreCommand('printList')"
              >
                打印列表
              </ElDropdownItem>
              <ElDropdownItem
                v-if="hasAccessByCodes(['fms:voucher:export'])"
                @click="handleMoreCommand('export')"
              >
                导出
              </ElDropdownItem>
              <ElDropdownItem
                ...
                "
                @click="handleMoreCommand('import')"
              >
                导入凭证
              </ElDropdownItem>
              <ElDropdownItem
      ...
      <template #voucherWord="{ row }">
        <ElButton link type="primary" @click="openVoucher(row)">
      ...
      <template #status="{ row }">
        <ElTag v-if="row.closingGenerated" type="info">结账生成</ElTag>
        <ElTag
          v-else
          :type="
