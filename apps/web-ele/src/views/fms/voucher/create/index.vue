...
const formData = reactive({
  ...
  voucherTime: dayjs().startOf('day').valueOf() as number | string,
  attachmentCount: 0,
}); // 凭证表单数据
...
function disableVoucherDate(date: Date) {
  return (
    ...
    dayjs(date).isBefore(`${currentMonth.value}-01`, 'day')
  );
}
...
function buildPayload(): FmsVoucherApi.SaveReq | undefined {
  ...
  if (
    ...
    ElMessage.warning('请完整填写凭证字、凭证号和凭证日期');
    return;
  }
  ...
  if (filledEntries.length < 2) {
    ElMessage.warning('凭证至少需要两条有效分录');
    return;
  }
...
function buildTemplateEntries(): FmsVoucherApi.VoucherEntry[] | undefined {
  const filledEntries = entries.value.filter((entry) => !isEntryEmpty(entry));
  if (filledEntries.length < 2) {
    ElMessage.warning('凭证模板至少需要两条有效分录');
    return;
  }
  fillEntryDigests(filledEntries);
  if (filledEntries.some((entry) => !validateEntry(entry, true))) {
...
function validateEntry(entry: VoucherEntryForm, amountRequired: boolean) {
  ...
  if (!entry.digest) {
    ElMessage.warning('请填写每条分录的摘要');
    return false;
  }
  const debitAmount = Number(entry.debitAmount || 0);
  const creditAmount = Number(entry.creditAmount || 0);
  if (debitAmount !== 0 && creditAmount !== 0) {
    ElMessage.warning('同一条分录不能同时填写借方和贷方金额');
    return false;
  }
  if (amountRequired && debitAmount === 0 && creditAmount === 0) {
    ElMessage.warning('请填写每条分录的借方或贷方金额');
    return false;
  }
  const auxiliaryTypeIds = subject.auxiliaryTypeIds || [];
  if (
...
/** 处理更多操作 */
function handleMoreCommand(command: 'applyTemplate' | 'saveTemplate') {
...
async function applyTemplate(template: FmsVoucherTemplateApi.VoucherTemplate) {
  ...
  if (unavailableSubject) {
    ElMessage.error('模板包含当前账套不可用的会计科目，暂不能套用');
    return;
  }
        ...
        >
          ...
          <template #dropdown>
            <ElDropdownMenu>
              ...
              <ElDropdownItem command="applyTemplate">使用模板</ElDropdownItem>
            </ElDropdownMenu>
          </template>
        </ElDropdown>
      </div>
      ...
      >
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <ElButton circle link title="插入分录">
                        <span class="icon-[ep--circle-plus]"></span>
                      </ElButton>
                      <template #dropdown>
                        <ElDropdownMenu>
                          <ElDropdownItem command="before">
                            从上方插入行
                          </ElDropdownItem>
                          <ElDropdownItem command="after">
                            从下方插入行
                          </ElDropdownItem>
                        </ElDropdownMenu>
                      </template>
        ...
        >
          ...
          <template v-if="formData.id">
            ...
            <ElTag :type="isApproved ? 'success' : 'warning'">
              {{ isApproved ? '已审核' : '待审核' }}
            </ElTag>
            <ElTag v-if="isClosingGenerated" type="info">结账生成凭证</ElTag>
          </template>
        </div>
        <div
          v-if="isApproved"
