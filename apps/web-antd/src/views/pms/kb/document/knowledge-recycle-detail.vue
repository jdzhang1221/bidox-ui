...
import { Alert, Breadcrumb, Button, Empty, Tag } from 'ant-design-vue';
...
<template>
  <div>
    ...
    >
      <div class="flex min-w-0 items-center gap-2">
        ...
        <Breadcrumb separator="/" class="min-w-0">
          <Breadcrumb.Item v-for="item in breadcrumbs" :key="item.key">
            <Button
    ...
    <Alert
      :closable="false"
      :message="`“${detail.root.name}”删除于 ${detail.root.deleteTime ? dayjs(detail.root.deleteTime).format('YYYY-MM-DD HH:mm') : '未知时间'}`"
      class="!mb-3"
      type="info"
    />
