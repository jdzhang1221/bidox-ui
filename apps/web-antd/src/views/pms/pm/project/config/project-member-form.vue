...
const [Modal, modalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    try {
      ...
      const newUserIds = values.userIds.filter(
        (userId) =>
          formType.value === 'update' ||
          !existingUserIds.value.includes(userId),
      );
      if (newUserIds.length === 0) {
        message.warning('所选用户均已是项目成员');
        return;
      }
      await updateProjectMemberList(
        projectId.value,
        newUserIds.map((userId) => ({ userId, level: values.level })),
      );
      message.success(
    ...
    <Form class="mx-4">
      ...
      <template #member>
        <div class="flex items-center gap-2">
          <Avatar :size="30" :src="currentMember?.avatar">
