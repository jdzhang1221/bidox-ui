...
import { Alert, Button, Empty } from 'antdv-next';
...
<template>
  <div>
    ...
    <Alert
      class="!mb-3"
      :message="`当前范围累计登记 ${report.totalHours} 小时`"
      show-icon
      type="info"
    />
    ...
    <Grid>
      <template #name="{ row }">
        ...
        <Button v-else type="link" @click="openWorkItem(row)">
          #{{ row.serialNumber }} {{ row.name }}
        </Button>
      </template>
      <template #empty>
        <Empty description="当前范围暂无工时记录" />
      </template>
    </Grid>
  </div>
  <!-- 工作项详情 -->
