  ...
  <Spin :spinning="loading">
    ...
    <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <DatePicker.RangePicker v-model:value="dateRange" class="!w-[240px]" />
        <Select
          ...
          "
        />
        <Button @click="resetRange">重置时间轴</Button>
      </div>
    </div>
    <Empty
    ...
    <div
      v-else
      class="max-h-[680px] min-w-[920px] overflow-auto border border-solid border-border"
      :style="{ '--timeline-width': `${timelineWidth}px` }"
    >
      <div
        class="sticky top-0 z-[4] grid min-h-[48px] grid-cols-[480px_var(--timeline-width)] border-0 border-b border-solid border-border bg-background"
      >
        <div
          class="sticky left-0 z-[5] grid min-w-0 grid-cols-[280px_100px_100px] overflow-hidden border-0 border-r border-solid border-border bg-background"
        >
          <span class="flex items-center px-3">名称</span>
          <span
            class="flex items-center border-0 border-l border-solid border-border px-2.5"
          >
            开始日期
          </span>
          <span
        ...
        <div
          class="relative flex items-center justify-between px-2.5 text-xs text-muted-foreground"
        >
          <span
            v-for="tick in timelineTicks"
            :key="tick.time"
            class="absolute translate-x-[-1px] border-0 border-l border-solid border-border pl-1"
      ...
      <div
        ...
        :key="row.key"
        class="grid min-h-[48px] grid-cols-[480px_var(--timeline-width)] border-0 border-b border-solid border-border"
        :class="{ 'bg-accent': row.group }"
      >
        <div
          class="sticky left-0 z-[2] grid min-w-0 grid-cols-[280px_100px_100px] overflow-hidden border-0 border-r border-solid border-border bg-inherit"
        >
          <div
          ...
          >
            ...
            />
            <Button v-if="row.item" type="link" @click="openWorkItem(row.item)">
        ...
        <div
          class="relative flex items-center justify-between px-2.5 text-xs text-muted-foreground [background-image:linear-gradient(to_right,hsl(var(--border))_1px,transparent_1px)] [background-size:10%_100%]"
        >
          <div
            class="absolute z-[1] h-full w-px bg-red-500"
          ...
          >
            ...
            <div
              class="absolute flex h-6 min-w-[18px] items-center justify-center overflow-hidden rounded bg-primary/100 text-white"
              :class="{ '!h-[18px] !bg-green-500': row.group }"
              :style="{
                left: `${getBarLeft(row)}%`,
