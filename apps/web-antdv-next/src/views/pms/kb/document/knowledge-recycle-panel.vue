...
// antdv-next Tabs 的 activeKey 为字符串，内部比较仍使用数值类型
...
<template>
  <div>
    ...
    <template v-else>
      ...
      >
        <Tabs.TabPane
          :key="String(PmsKnowledgeObjectType.DOCUMENT)"
          :tab="`文档 (${countByType(PmsKnowledgeObjectType.DOCUMENT)})`"
        />
        <Tabs.TabPane
          :key="String(PmsKnowledgeObjectType.FOLDER)"
          :tab="`文件夹 (${countByType(PmsKnowledgeObjectType.FOLDER)})`"
        />
        <Tabs.TabPane
          :key="String(PmsKnowledgeObjectType.FILE)"
